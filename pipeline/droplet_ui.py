...
# ─────────────────────────────────────────────────────────────────────────────
def launch_ui(czi_path=None):
    """Build and display the interactive droplet-analysis UI.

    Parameters
    ----------
    czi_path : str, optional
        Pre-fill the file path text box (useful when calling from Google Colab
        after uploading a file with ``google.colab.files.upload()``).

    Returns
    -------
    S : dict  with keys df, norm, pixel_size, etc.
    """
