# -*- coding: utf-8 -*-
"""
metric_d_volume_fraction.py
Metric D: volume fraction per class (spatial occupancy).
  vol_frac[c] = sum(volume_um3 for class c) / sum(volume_um3 for all droplets)
Low-N classes (N < MIN_N) flagged with '* N=X' above the bar (bar plots)
or hollow markers (line plots).

Outputs:
  fig1D_page_comparison.png
  fig2D_crossgroup_day4.png
  fig3D_timecourse.png
  fig3D_byday.png
"""
...
from utils import (
    ...
    MIN_N, load_condition, class_volume_fractions, class_counts,
...
def fig1D():
    ...
    fracs    = [class_volume_fractions(df) for df in datasets]
    ...
    ax.set_xlabel("Class")
    ax.set_ylabel("Fraction of total condensate volume")
    ax.set_title("Figure 1D -- Volume fraction: PAGE (day 3) vs heated conditions (day 4)")
    ax.yaxis.set_major_formatter(mticker.PercentFormatter(xmax=1, decimals=0))
    ax.set_xlim(-0.6, 15.6)
    fig.tight_layout()
    save_figure(fig, "fig1D_page_comparison.png")
    plt.close(fig)
...
def fig2D():
    ...
    fig.suptitle("Figure 2D -- Volume fraction per class: cross-group comparison (day 4)", fontsize=11)

    for row in range(2):
        for col in range(2):
            lbl, folder = FIG2_LAYOUT[row][col]
            df   = load_condition(folder)
            frac = class_volume_fractions(df)
    ...
    fig.tight_layout()
    save_figure(fig, "fig2D_crossgroup_day4.png")
    plt.close(fig)
...
def fig3D():
    ...
    fig.suptitle("Figure 3D -- Volume fraction over time (per group)", fontsize=11)
    ...
    fig.tight_layout()
    save_figure(fig, "fig3D_timecourse.png")
    plt.close(fig)
...
def fig3D_byday():
    ...
    fig.suptitle("Figure 3D (by day) -- Volume fraction vs time (per group)", fontsize=11)
    ...
    for idx, (gname, timepoints) in enumerate(FIG3_GROUPS.items()):
        ...
        for c in CLASSES:
            fracs_c  = [class_volume_fractions(df)[c] for df in tp_datasets]
    ...
    save_figure(fig, "fig3D_byday.png")
    plt.close(fig)
